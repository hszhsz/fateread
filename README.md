...
- **多流派**：三大命理流派独立 Agent — 子平八字、紫微斗数、盲派命理
- **辩论模式**：可选开启的三派会诊 + 辩论协调，默认单流派执行以节省 Token
...
```bash
npm run start -- --school ziping    # 子平八字（默认）
npm run start -- --school ziwei     # 紫微斗数
npm run start -- --school mangpai   # 盲派命理
```
