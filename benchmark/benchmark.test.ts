...
import type { BaziChart, PaipanInput } from '../src/core/types.js';
import { TIAN_GAN, DI_ZHI, STEM_ELEMENT } from '../src/core/constants.js';
...
function loadCases(school: string): BenchmarkCase[] {
  const raw = readFileSync(resolve(CASES_DIR, `${school}.json`), 'utf-8');
  return JSON.parse(raw) as BenchmarkCase[];
}

function runPaipan(c: BenchmarkCase): BaziChart {
  return paipan(c.input);
}

function p(stem: string, branch: string): string {
  return stem + branch;
}
...
describe('盲派命理 (Mangpai) Benchmark', () => {
  ...
  test('同八字异性别四柱相同但大运方向相反', () => {
    // Same birth data, different gender
    const maleInput: PaipanInput = { year: 1968, month: 10, day: 18, hour: 23, minute: 45, gender: 'male', longitude: 120 };
    const femaleInput: PaipanInput = { year: 1968, month: 10, day: 18, hour: 23, minute: 45, gender: 'female', longitude: 120 };

    const chartM = paipan(maleInput);
    const chartF = paipan(femaleInput);
  ...
  test('大运包含合理数量的运程（6-12步）', () => {
    for (const c of cases) {
      const chart = runPaipan(c);
      expect(chart.daYun.pillars.length).toBeGreaterThanOrEqual(6);
      expect(chart.daYun.pillars.length).toBeLessThanOrEqual(12);
      for (const dp of chart.daYun.pillars) {
        expect(TIAN_GAN).toContain(dp.pillar.stem);
        expect(DI_ZHI).toContain(dp.pillar.branch);
      }
    }
  });
